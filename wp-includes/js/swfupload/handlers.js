    ...
    jQuery('a.undo', item).click(function () {
        ...
        jQuery.ajax({
            ...
            success: function (data, textStatus) {
                ...
                item.css({backgroundColor: '#ceb'}).animate({backgroundColor: '#fff'}, { queue: false, duration: 500, complete: function () {
...
function itemAjaxError(id, html) {
    ...
    item.html('<div class="error-div">'
        ...
        + html
        + '</div>');
    item.find('a.dismiss').click(function () {
        jQuery(this).parents('.media-item').slideUp(200, function () {
            jQuery(this).remove();
        })
    });
}
