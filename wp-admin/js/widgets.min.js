...
!function (a) {
    wpWidgets = {init: function () {
        ...
        a("#widgets-right .sidebar-name").click(function () {
        ...
        }), a("#widget-list").children(".widget").draggable({connectToSortable: "div.widgets-sortables", handle: "> .widget-top > .widget-title", distance: 2, helper: "clone", zIndex: 100, containment: "document", start: function (b, e) {
            ...
            b && a(b).hide(), b = ""
        }}), g.sortable({placeholder: "widget-placeholder", items: "> .widget", handle: "> .widget-top > .widget-title", cursor: "move", distance: 2, containment: "document", start: function (b, c) {
            ...
            return l.hasClass("deleting") ? (wpWidgets.save(l, 1, 0, 1), l.remove(), void 0) : (f = l.find("input.add_new").val(), g = l.find("input.multi_number").val(), l.attr("style", "").removeClass("ui-draggable"), c = "", f && ("multi" === f ? (l.html(l.html().replace(/<[^<>]+>/g, function (a) {
                return a.replace(/__i__|%i%/g, g)
            })), l.attr("id", m.replace("__i__", g)), g++, a("div#" + m).find("input.multi_number").val(g)) : "single" === f && (l.attr("id", "new-" + m), b = "div#" + m), wpWidgets.save(l, 0, 0, 1), l.find("input.add_new").val("")), h = l.parent(), h.parent().hasClass("closed") && (h.parent().removeClass("closed"), i = h.children(".widget"), i.length > 1 && (j = i.get(0), k = l.get(0), j.id && k.id && j.id !== k.id && a(j).before(l))), f ? l.find("a.widget-action").trigger("click") : wpWidgets.saveOrder(h.attr("id")), void 0)
        }, activate: function () {
            ...
            var d = a(c.sender);
            return this.id.indexOf("orphaned_widgets") > -1 ? (d.sortable("cancel"), void 0) : (d.attr("id").indexOf("orphaned_widgets") > -1 && !d.children(".widget").length && d.parents(".orphan-sidebar").slideUp(400, function () {
                a(this).remove()
            }), void 0)
        }}).sortable("option", "connectWith", "div.widgets-sortables"), a("#available-widgets").droppable({tolerance: "pointer", accept: function (b) {
        ...
        b = a(b), a(".spinner", b).show(), f = {action: "save-widget", savewidgets: a("#_wpnonce_widgets").val(), sidebar: g}, c && (f.delete_widget = 1), h += "&" + a.param(f), a.post(ajaxurl, h, function (f) {
            var g;
            ...
            })) : b.remove()) : (a(".spinner").hide(), f && f.length > 2 && (a("div.widget-content", b).html(f), wpWidgets.appendTitle(b))), e && wpWidgets.saveOrder()
        })
